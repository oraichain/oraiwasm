...
macro_rules! init_dealer {
    ...
    ($deps:expr, $addresses:expr, $dealer:expr, $threshold:expr, $sample:expr) => {
        ...
        for i in 0..$dealer {
            ...
            let msg = ExecuteMsg::ShareDealer {
...
fn share_dealer() {
    ...
    let query_members = QueryMsg::GetMembers {
        limit: Some(5),
        offset: Some("orai17zr98cwzfqdwh69r8v5nrktsalmgs5sawmngxz".to_string()),
        order: None,
    };
    ...
    //     limit: Some(5),
    //     offset: Some("orai14v5m0leuxa7dseuekps3rkf7f3rcc84kzqys87"),
    //     order: None,
    // };
    // let dealers: Vec<Member> =
    //     from_json(&query(deps.as_ref(), mock_env(), query_dealers).unwrap()).unwrap();
...
fn request_round() {
    ...
    let members: Vec<Member> = from_json(
        &query(
            deps.as_ref(),
            mock_env(),
            QueryMsg::GetMembers {
    ...
    for i in 1u64..3u64 {
        ...
        let verified: bool =
            from_json(&query(deps.as_ref(), mock_env(), verify_msg.clone()).unwrap()).unwrap();
...
fn test_verify_signed_sig() {
    ...
    let signed_signature = vec![
        76, 168, 2, 182, 84, 102, 63, 140, 38, 174, 209, 31, 145, 225, 72, 164, 105, 130, 106, 227,
        18, 26, 207, 5, 174, 33, 179, 172, 120, 166, 185, 98, 39, 165, 26, 67, 140, 62, 113, 192,
        47, 57, 75, 108, 5, 179, 181, 85, 240, 65, 135, 194, 83, 96, 245, 154, 117, 81, 66, 244,
        109, 22, 41, 60,
    ];
    ...
    let pubkey = vec![
        3, 86, 137, 186, 150, 244, 223, 81, 172, 126, 86, 95, 129, 170, 108, 60, 116, 171, 244,
        212, 116, 145, 214, 46, 197, 119, 64, 140, 94, 134, 43, 54, 140,
    ];
    println!("signature length {:?}", signed_signature.len());

    let verified =
        secp256k1_verify(result.as_slice(), signed_signature.as_slice(), &pubkey).unwrap();
