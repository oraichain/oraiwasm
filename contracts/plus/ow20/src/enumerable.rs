...
use crate::state::{allowances_read, balances_prefix_read, calc_range_start_human};
...
pub fn query_all_accounts(
    ...
    let accounts = balances_prefix_read(deps.storage)
        .range(start.as_deref(), None, Order::Ascending)
        .take(limit)
        .map(|(k, _)| Ok(api.addr_humanize(&CanonicalAddr::from(k))?.to_string()))
...
mod tests {
    ...
    fn query_all_allowances_works() {
        ...
        let env = mock_env();
        do_init(deps.as_mut(), &owner, Uint128::from(12340000u128));
        ...
        let allowances = query_all_allowances(
            deps.as_ref(),
            owner.clone(),
            Some(Addr::unchecked(allow.spender.as_str())),
            Some(10000),
        )
        .unwrap();
        assert_eq!(allowances.allowances.len(), 1);
    ...
    fn query_all_accounts_works() {
        ...
        do_init(deps.as_mut(), &acct1, Uint128::from(12340000u128));

        // put money everywhere (to create balanaces)
        let info = mock_info(acct1.as_str(), &[]);
        let env = mock_env();
        execute(
            deps.as_mut(),
            env.clone(),
        ...
        let accounts = query_all_accounts(
            deps.as_ref(),
            Some(Addr::unchecked(accounts.accounts[1].as_str())),
            Some(1),
        )
        .unwrap();
        ...
        let accounts = query_all_accounts(
            deps.as_ref(),
            Some(Addr::unchecked(accounts.accounts[0].as_str())),
            Some(777),
        )
        .unwrap();
