...
impl Into<Expiration> for OldExpiration {
    fn into(self) -> Expiration {
        match self {
            ...
            OldExpiration::AtTime { time } => Expiration::AtTime(Timestamp::from_seconds(time)),
...
pub mod testing {
    ...
    pub fn generate_v01_test_data<S: Storage, A: Api>(storage: &mut S, api: &A) -> StdResult<()> {
        ...
        set_contract_version(storage, "crates.io:ow20-base", "v0.1.0")?;
        ...
        let user1 = api.addr_canonicalize("user1")?;
        let user2 = api.addr_canonicalize("user2")?;
        let mut balances = crate::state::balances(storage);
        balances.save(user1.as_slice(), &Uint128::from(123456u128))?;
        balances.save(user2.as_slice(), &Uint128::from(654321u128))?;

        let spender1 = api.addr_canonicalize("spender1")?;
        let spender2 = api.addr_canonicalize("spender2")?;
