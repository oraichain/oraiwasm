...
use cosmwasm_std::{coin, coins, HumanAddr, Uint128, Coin};
...
fn exec_update_service_contract() {
    ...
    println!("1 test exec_update_service_contract get service info contract: {:?} {:?} ", provider_contract, service_info);

    /*
     * testcase 2
     * exec handle service contract error author
     */
    let exec_update_service1 = app.execute_contract(
        SENDER1,
        &provider_contract,
    ...
    assert_eq!(exec_update_service1.to_string(), ContractError::Unauthorized {}.to_string());
    println!("2 test exec_update_service_contract {:?} ", exec_update_service1);
    ...
    let service_info3: ServiceInfo = app
        ...
        .unwrap();
    if service_info3.contracts.dsources.eq(&vec![dsource_new.to_owned()])
    ...
    app.execute_contract(
        ...
        &HandleMsg::UpdateConfig {
            ...
            bound_executor_fee: Some(new_coin.to_owned())
        },
        &[],
    ).ok();
    ...
    if service_info4.owner.eq(&PROVIDER_OWNER_2)
        && service_info4.fee_contract.eq(&AIORACLE_SERVICE_FEES_OWNER_2)
        && service_info4.bound_executor_fee.eq(&new_coin) 
    {
        assert!(true);
    } else {
        assert!(false);
...
fn query_service_contract() {
    ...
    println!("1 test query_service_contract query contract {:?} ", service_contract);
    ...
    println!("2 test query_service_contract query service_fee {:?} ", service_fee);
    ...
    let service_part_fee: Coin = app
        ...
        .unwrap();
    if service_part_fee.denom.eq(&DENOM)
        && service_part_fee.amount.eq(&FEE_2)
    {
        assert!(true);
    } else {
        assert!(false);
    }
    println!("3 test query_service_contract query service_part_fee {:?} ", service_part_fee);
    ...
    let service_bound_executor_fee: Coin = app
        ...
        .query_wasm_smart(
            &provider_contract,
            &QueryMsg::GetBoundExecutorFee {
                service: SERVICE_NAME.to_string()
            },
        )
        .unwrap();
    if service_bound_executor_fee.denom.eq(&DENOM)
        && service_bound_executor_fee.amount.eq(&Uint128(1))
    {
        assert!(true);
    } else {
        assert!(false);
    }
    println!("4 test query_service_contract query service_part_fee {:?} ", service_bound_executor_fee);
}
