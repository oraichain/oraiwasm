use crate::{
    ...
    msg::{HandleMsg, InitMsg, Member, QueryMsg},
    state::Config,
};
...
macro_rules! init_dealer {
    ...
    ($deps:expr, $addresses:expr, $dealer:expr, $threshold:expr, $sample:expr) => {
        ...
        for i in 0..$dealer {
            println!("index: {}", i);
            println!("commit list len: {}", commits_list.len());
            println!("row list len: {}", rows_list.len());
            let info = mock_info($addresses[i], &vec![]);
            let msg = HandleMsg::Msg(AiOracleMembersMsg::ShareDealer {
...
macro_rules! init_row {
    ($deps:expr, $members:expr, $dealers:expr) => {
        ...
        for member in &$members {
            ...
            let msg = HandleMsg::Msg(AiOracleMembersMsg::ShareRow {
                share: SharedRowMsg {
...
fn share_dealer() {
    ...
    let ret: Config = from_binary(
        &query(
            deps.as_ref(),
            mock_env(),
            QueryMsg::Msg(AiOracleMembersQuery::GetConfigInfo {}),
        )
        .unwrap(),
    )
    .unwrap();
    ...
    // test query members
    let query_members = QueryMsg::Msg(AiOracleMembersQuery::GetMembers {
        limit: Some(5),
...
fn test_reset() {
    ...
    let _config: Config = from_binary(
        &query(
            deps.as_ref(),
            mock_env(),
            QueryMsg::Msg(AiOracleMembersQuery::GetConfigInfo {}),
    ...
    let members: Vec<Member> = from_binary(
        &query(
            deps.as_ref(),
            mock_env(),
            QueryMsg::Msg(AiOracleMembersQuery::GetMembers {
    ...
    // update threshold
    let threshold_msg = HandleMsg::Msg(AiOracleMembersMsg::Reset {
        threshold: Some(3),
        members: None,
    });
    handle(
