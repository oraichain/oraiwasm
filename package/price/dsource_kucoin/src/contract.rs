...
use crate::msg::{Coin, Data, HandleMsg, InitMsg, Kucoin, QueryMsg, SpecialQuery};
use cosmwasm_std::{
...
fn query_data(deps: Deps, _input: String) -> StdResult<Binary> {
    ...
    for symbol in list_symbols {
        ...
        let price = query_kucoin(deps, symbol);
...
fn query_kucoin(deps: Deps, symbol: &str) -> String {
    let price_str = query_price(
        deps,
        format!("https://api.kucoin.com/api/v1/prices?currencies={}", symbol),
    );
    if price_str == "none" {
        return String::from("none");
    }
    let result: Result<Kucoin, StdError> = from_slice(price_str.as_bytes());
    if result.is_err() {
        return String::from("none");
    }
    let price = match result.unwrap().data {
        Coin::BTC(price) => price,
        Coin::ETH(price) => price,
        Coin::BNB(price) => price,
        Coin::XRP(price) => price,
        Coin::DOGE(price) => price,
        Coin::USDT(price) => price,
        Coin::LINK(price) => price,
        Coin::UNI(price) => price,
        Coin::USDC(price) => price,
        Coin::BUSD(price) => price,
        Coin::ORAI(price) => price,
        Coin::DAI(price) => price,
    };
    return price;
}
