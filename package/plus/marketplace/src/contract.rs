...
use cosmwasm_std::{
    ...
    Fraction, MessageInfo, Order, Pair, Response, StdResult, WasmMsg,
};
...
pub fn try_buy(
    ...
    let sent_fund = info.funds[0].clone();
    ...
    if !sent_fund.denom.eq("orai") {
        ...
        let rate = denom_rate.unwrap();

        // calculate the exact amount by ratio
        amount = amount.multiply_ratio(rate.numerator(), rate.denominator());
    }
    ...
    let seller_result = deps.api.addr_humanize(&off.seller);
    ...
    let contract_addr_result = deps.api.addr_humanize(&off.contract_addr);
    if contract_addr_result.is_err() {
        return Err(ContractError::InvalidContractAddr {});
    }
    let contract_addr = contract_addr_result?;
...
pub fn try_withdraw(
    ...
    if off.seller == deps.api.addr_canonicalize(&info.sender.as_str())? {
        ...
        let transfer_cw721_msg = Cw721ExecuteMsg::TransferNft {
            recipient: deps.api.addr_humanize(&off.seller)?.to_string(),
            token_id: off.token_id.clone(),
        };
...
mod tests {
    ...
    fn sell_offering_happy_path() {
        ...
        let _res_second = execute(deps.as_mut(), mock_env(), info.clone(), msg_second).unwrap();
