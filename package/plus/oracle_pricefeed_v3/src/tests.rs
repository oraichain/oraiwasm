use crate::{
    contract::{get_all_members, get_input, handle, init, query, query_current},
    msg::{
...
fn request_round() {
    ...
    while current_round_result.is_ok() {
        ...
        for contributor in contributors {
            ...
            println!(
                ...
                current_round.raw_input.to_base64()
            );
            let msg = get_input(
                current_round.raw_input.as_slice(),
                &current_round.round.to_be_bytes(),
            );
    ...
    for latest_round in latest_rounds {
        ...
        println!(
            ...
            latest_round.round,
            latest_round.raw_input.to_base64(),
            latest_round.combined_sig_hash.unwrap().to_base64()
        );
    }
