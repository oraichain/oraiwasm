...
use blsdkg::{
    derive_randomness, get_hash, hash_g2, PublicKey, PublicKeySet, PublicKeyShare, Signature,
...
pub fn update_share_sig(
    ...
    let msg = get_input(share_data.raw_input.as_slice(), &round_key);
    ...
    if share_data.sigs.len() as u16 > threshold {
        ...
        if verifed {
            ...
            share_data.combined_sig_hash = Some(Binary::from(randomness));
        } else {
...
pub fn get_input(input: &[u8], round: &[u8]) -> Vec<u8> {
    let mut final_input = input.to_vec();
    final_input.extend(round);
    get_hash(final_input.as_slice()).to_vec()
}
...
fn verify_round(deps: Deps, round: u64) -> Result<bool, ContractError> {
    ...
    let msg = get_input(share_data.raw_input.as_slice(), &round.to_be_bytes());
    let hash_on_curve = hash_g2(msg);
    if let Some(combined_pubkey_bin) = share_data.combined_pubkey {
        if let Some(combined_sig_bin) = share_data.combined_sig {
            ...
            let mut verifed = combined_pubkey.verify_g2(&combined_sig, hash_on_curve);
            let sig_hash = Binary::from(derive_randomness(&combined_sig));
            if let Some(combined_sig_hash) = share_data.combined_sig_hash {
                verifed = sig_hash.eq(&combined_sig_hash);
            }
            return Ok(verifed);
        } else {
            return Ok(false);
