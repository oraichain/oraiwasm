...
use crate::errors::{Error, FromBytesError, FromBytesResult, Result};
...
mod tests {
    ...
    fn dkg_bls_round() {
        ...
        for (rows, commits) in rows_list.iter().zip(commits_list) {
            ...
            for m in 1..=node_num {
                ...
                let row_poly = Poly::from_bytes(base64::decode(rows[m - 1]).unwrap()).unwrap();
                println!(
                    "share row: {}",
                ...
                let row_commit =
                    Commitment::from_bytes(base64::decode(commits[m]).unwrap()).unwrap();
