...
pub fn verify(
    ...
    let msg_on_g2 = verify_step1(round, previous_signature, user_input);
...
#[doc(hidden)]
pub fn verify_step1(round: u64, previous_signature: &[u8], user_input: &[u8]) -> G2Affine {
    let msg = message(round, previous_signature, user_input);
    msg_to_curve(&msg)
}
...
fn message(current_round: u64, prev_sig: &[u8], user_input: &[u8]) -> Vec<u8> {
...
mod tests {
    ...
    fn verify_works() {
        ...
        // good
        let result = verify(&pk, round, &previous_signature, "".as_bytes(), &signature).unwrap();
