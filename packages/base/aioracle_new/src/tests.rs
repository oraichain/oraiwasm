...
fn test_update_state() {
    ...
    let msg = ExecuteMsg::SetState(StateMsg {
        owner: Some(Addr::unchecked("hey")),
        dsources: None,
        tcases: Some(vec![Addr::unchecked("hlhlhlhlhlh")]),
    });
...
fn test_hash() {
    ...
    let dsource_result: DataSourceResultMsg = from_json(dsource_result_str.as_bytes()).unwrap();
